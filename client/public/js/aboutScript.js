 ...
 (function () {
   ...
   TabsNav.prototype.options = {
     ...
     onOpenBarsUpdate: function (anim, idx, tab) {
       return false;
     }
   };
   ...
   TabsNav.prototype._openTab = function (tab) {
     ...
     var animeTabs = {
         targets: this.options.movable === 'all' ? this.DOM.tabs : this.DOM.tabs[this.current]
       },
       animeBars = {
         targets: this.options.movable === 'all' ? this.DOM.bars : this.DOM.bars[this.current]
       },
       animeTabsDelay = function (target, index, cnt) {
         if (cnt === 1 || self.options.animedelay === 0) {
           ...
           if (self.current >= middle) {
             return index <= self.current ? index * self.options.animedelay : (total - index - 1) * self.options
               .animedelay;
           } else {
             return index < self.current ? index * self.options.animedelay : (total - index - 1) * self.options
               .animedelay;
       ...
       animeTabsTranslation = function (target, index, cnt) {
         if (index === self.current || cnt === 1) {
           ...
           return index > self.current ? self.dim.win - (self.dim.position + self.dim.measure) - pixels : -1 *
             self.dim.position + pixels;
         }
       },
     ...
     if (this.extraEl) {
       var animeExtra = {
         ...
         animeExtraTranslation = this.options.layout === 'vertical' ? this.dim.win - (this.dim.position + this
           .dim.measure) + Math.abs(extraBounds.left - this.dim.position) + this.dim.measure : -1 * this.dim.position;
   ...
   TabsNav.prototype._closeTab = function (tab) {
     ...
     var self = this,
       ...
       animeTabsDelay = function (target, index, cnt) {
         return cnt === 1 || self.options.animedelay === 0 ? 0 : Math.abs(self.current - index) * self.options
           .animedelay;
       };
   ...
   TabsNav.prototype.show = function (callback) {
     ...
     this.DOM.tabs.forEach(function (tab) {
       ...
       title.style.transform = self.options.layout === 'vertical' ? 'translateX(10) rotate(-90)' :
         'translateY(10)';
     });
     ...
     var animeBars = {
       ...
       complete: function () {
         ...
         self.DOM.tabs.forEach(function (tab) {
           ...
           title.style.transform = self.options.layout === 'vertical' ? 'translateX(0) rotate(-90)' :
             'translateY(0)';
         });
   ...
   TabsNav.prototype._resize = function () {
     ...
     if (this.isOpen) {
       ...
       this.DOM.tabs.forEach(function (tab, idx) {
         if (self.options.movable === 'all' || self.options.movable === 'single' && idx === self.current) {
           ...
           if (idx === self.current) {
             ...
             tVal = idx > self.current ? self.dim.win - (self.dim.position + self.dim.measure) : -1 * self
               .dim.position;
             sVal = 1;
           }
       ...
       if (this.extraEl) {
         this.extraEl.style.transform = translateStr + '(' + (this.options.layout === 'vertical' ? this.dim.win -
           (this.dim.position + this.dim.measure) : -1 * this.dim.position) + 'px)';
       }
     }
   };
...
function openTabCallback(anim, idx, tab) {
  if (anim.progress > 60 && !isContentShown) {
    ...
    content.img.style.opacity = content.title.style.opacity = content.deco.style.opacity = content.box.style.opacity =
